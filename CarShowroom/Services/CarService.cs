...
{
    private BotDbContext _dbContex;
    private ILogger<UserService> _logger;

    public CarService(BotDbContext context,ILogger<UserService> logger)
    {
        _dbContex=context;
        _logger=logger;
    }

    public async Task<CarModel?> GetCarByNameAsync(string? name)
    {
        ArgumentNullException.ThrowIfNull(name);
        try
        {
            var car= await _dbContex.Car.FirstOrDefaultAsync(x=>x.Name==name);
   ...
   public async  Task<bool> AddCarAsync(List<CarModel> carModels)
   {

        try
        {
        await _dbContex.Car.AddRangeAsync(carModels);
        await _dbContex.SaveChangesAsync();
        _logger.LogInformation("Car bazaga qoshildi");
        return true;
        }   
        catch (System.Exception)
        {
            _logger.LogInformation(" Car Saqlanmadi");
            return false;
        }
  }


  public async Task<CarModel[]> FindCarByBrand(string name)
  {
        try
        {
            var car2=  _dbContex.Car.Where(x=>x.Brand==name).ToArray();
            return car2;
        }
        catch (System.Exception)
        {
            
            throw new Exception("Bunday brentdagi mashina topilmadi");
        }
  }
  ...
  public async Task<bool> SaveCarDataAsync()
  {
    ...
    return await AddCarAsync(carModels);  
   
   }
//    public async Task<CarModel[]> GetCarsAsync()
//    {
//     return await _dbContex.Car.ToArrayAsync();
//    }
